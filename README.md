...
**Current features:**
- **Verbose debug**: A **Verbose debug** checkbox (unchecked by default) controls visibility of: the build version line, the "What was sent to the LLM" section, and the "Model XML sent to LLM" section. Check it to see version number and full request payload when debugging.
- **Build version**: When verbose debug is on, the view shows **ArchiGPT v1.0.0.&lt;qualifier&gt;** (e.g. v1.0.0.20250304120000 after a build). Use this to confirm you are running the latest plugin.
...
- **What was sent (visible above the response)**: When you click **Ask ArchiGPT**, the exact payload is shown **directly above** the LLM response: **(1) What was sent to the LLM (last request)** — prompt, selection context, and model XML length (with a short preview); **(2) Model XML sent to LLM (exact payload)** — the XML actually sent. Then **LLM response:** with the reply (starting with "Sent to LLM: X chars total (Y chars model XML). ---"). So you always see what was sent right above the analysis or import result.
