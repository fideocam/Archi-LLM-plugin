...
- **Model as context**: The open ArchiMate model is serialized to XML (elements, relationships, views and diagrams) and sent to the LLM with every request so it can avoid duplicates and describe the actual content. Up to 500,000 characters of model XML are sent (truncation message if larger).
- **What was sent**: Before every response (analysis or import), the response box shows **what was sent to the LLM**: your prompt, selection context (if any), and the full model XML supplied. Then the analysis result or import message.
