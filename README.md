...
- **Send** — On the **ArchiGPT** tab, type a prompt and click **Ask ArchiGPT** or press **Enter** (Shift+Enter for a new line). The **Tools** tab has its own prompt and reply; choosing a tool or running it does not change the ArchiGPT tab. Click **Stop ArchiGPT** (or press Enter again) to cancel a request.
- **Tool library** — Open the **Tools** tab. Choose a **Role**, then a **Task**. **Solution architect** is one service or system slice. **Technology architect** is hosts, paths, runtimes, and locations (redundancy, security, continuity, and technology choice as this model already shows them). **Enterprise architect** is landscape echo gaps and capability / motivation / migration fragments. **Model hygiene** is catalog versus canvas. The prompt and the reply stay on that tab. Hygiene, view, technology, EA, and “suggest a pattern” stay report-only; **Add …** tasks import a fragment via CHANGES JSON.
- **Ollama server** — Open the **Settings** tab to see the current model name, set the Ollama URL (default `http://localhost:11434`), pick a model, and **Refresh list**. Click **Preferences…** (or **ArchiGPT → ArchiGPT Preferences…**) to test the connection.
- **Context size** — On the **Settings** tab, ArchiGPT asks this Ollama server (`POST /api/show`) for the selected model's architecture maximum and shows it as **Ollama max**. That value is not invented by the plugin. Requests default to 32 768 tokens. **Use model max** sends the architecture limit as `num_ctx`, which allocates a KV cache of that size and can time out or run out of GPU memory — even on a large server. If calls fail, uncheck it and try 32768 or 65536.
...
- [Company metamodel](docs/CompanyMetamodel.md) — templates vs living reference, specializations, and later comparison in ArchiGPT.
