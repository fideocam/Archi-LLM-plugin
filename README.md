...
- **Model as context**: The open ArchiMate model is serialized to XML (elements, relationships, views and diagrams) and sent to the LLM with every request so it can avoid duplicates and describe the actual content. **Relevant parts first**: If you select a folder, view, or element in Archi, that context is sent *first* in the XML (selected folder's contents, selected view's diagram, or the view(s) that contain the selected element), so the LLM always sees what you're working on within the 24k limit. To fit **Ollama’s context limit** (often 4k–8k tokens with default settings), at most **24,000 characters** of model XML are sent; if the model is larger, the rest is truncated after the priority content. The plugin requests a 32k-token context from Ollama when available (increase in **Ollama → Settings** or via `OLLAMA_NUM_CTX` if the model still doesn’t “see” the XML).
- **What was sent**: Before every response (analysis or import), the response box shows **what was sent to the LLM**: your prompt, selection context (if any), **model XML length** (and truncation notice if applied), and the XML supplied. Then the analysis result or import message.
