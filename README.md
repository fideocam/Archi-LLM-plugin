...
- **Send** — On the ArchiGPT tab, press **Enter** (Shift+Enter for a new line). On the **Tools** tab, click **Ask ArchiGPT**. Click **Stop ArchiGPT** (or press Enter again) to cancel a request.
- **Tool library** — Open the **Tools** tab. The **Tool** list is grouped as **Tidy** (duplicate names, relationships not on any diagram), **View** (this canvas, a business-service diagram, or tech architecture for a system), **Pattern** (service sandwich and similar fragments), and **EA** (echo gaps: a related element or relationship already exists in one part of the model but not for peers or other views). Choosing a tool fills the prompt on the ArchiGPT tab (you can still edit it). Tidy, View, EA, and “suggest patterns” stay report-only; **Pattern** items that start with “Add …” import a fragment via CHANGES JSON.
