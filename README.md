...
- **Send** — Click **Ask ArchiGPT** on the ArchiGPT tab or the Tools tab. On either prompt box, press **Enter** to send (Shift+Enter for a new line). Click **Stop ArchiGPT** (or press Enter again) to cancel a request.
- **Tool library** — Open the **Tools** tab. There are two lists: **Solution architect** (Tidy, View, and Pattern tools for a service or system slice) and **EA** (echo gaps: a related element or relationship already exists in one part of the model but not for peers or other views). Choosing a tool from one list clears the other and shows the prompt on that tab (you can still edit it). Tidy, View, EA, and “suggest patterns” stay report-only; **Pattern** items that start with “Add …” import a fragment via CHANGES JSON.
